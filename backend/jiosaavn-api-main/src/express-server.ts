...
app.get('/api/songs', (req, res) => {
  ...
  return handleRequest(req, res, () =>
    parsedLink ? songService.getSongByLink(parsedLink) : songService.getSongByIds({ songIds: ids || '' })
  )
})
...
app.get('/api/albums', (req, res) => {
  ...
  return handleRequest(req, res, () => (link ? albumService.getAlbumByLink(link) : albumService.getAlbumById(id)))
})
...
app.get('/api/playlists', (req, res) => {
  ...
  return handleRequest(req, res, () =>
    link
      ? playlistService.getPlaylistByLink({ token: link, page, limit })
      : playlistService.getPlaylistById({ id, page, limit })
  )
})
...
app.get('/api/artists', (req, res) => {
  ...
  const link = req.query.link as string
  const page = Number.parseInt((req.query.page as string) || '0', 10)
  const songCount = Number.parseInt((req.query.songCount as string) || '10', 10)
  const albumCount = Number.parseInt((req.query.albumCount as string) || '10', 10)
...
app.get('/api/artists/:id', (req, res) => {
  ...
  return handleRequest(req, res, () =>
    artistService.getArtistById({ artistId, page, songCount, albumCount, sortBy, sortOrder })
  )
})
